...
import { searchShoesSuccess, searchShoesFailure } from '../actions/action-creators';
...
export const searchShoesEpic = (action$, state$) => state$.pipe(
    ...
    map(([prev, current]) => {
        ...
        return ({
            appendToPrevious: shouldAppendNewResultsToPrevious(prev, current),
            query: buildSearchParams(page, search, categoryId)
        })
    }),
    switchMap(({ appendToPrevious, query }) => ajax.getJSON(`${PRODUCTS_LIST_URL}?${query}`).pipe(
        retry(DEAFULT_RETRY_AMOUNT),
        map(items => searchShoesSuccess({ items, appendToPrevious, hasMore: checkIfHasMore(items) })),
        catchError(e => of(searchShoesFailure(e))),
    )),
);
